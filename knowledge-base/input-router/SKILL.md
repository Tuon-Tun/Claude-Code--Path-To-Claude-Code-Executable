...
5. **Capture JTBD signals:** Populate `JTBD_Candidates` when the input implies a performer, situation, motivation, or expected outcome. Mark missing fields as `[NEEDS_CLARIFICATION]`; Product Framing will refine them into single-sentence job stories later.
...
12. **Initialize Clarification_Confirmed:** Always set `Clarification_Confirmed` to `false` in the Step 1 JSON output. Never set it to `true` — that field is set only by the orchestrator in Step 1.5 after the user has explicitly approved the clarified JSON.
13. **Output only JSON:** Return valid JSON only. Do not add commentary, markdown fences, or any text outside the JSON object.
